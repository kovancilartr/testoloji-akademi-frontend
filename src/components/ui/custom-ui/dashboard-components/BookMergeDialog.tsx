...
export function BookMergeDialog({ isOpen, onOpenChange, selectedProjectIds, projects }: BookMergeDialogProps) {
    ...
    return (
        <>
            {/* FLOATING PAGE PREVIEW - Locked to screen bottom-right */}
            {isOpen && (
                <div
                    onPointerDown={(e) => e.stopPropagation()}
                    onClick={(e) => e.stopPropagation()}
                    className="fixed bottom-10 right-10 w-[300px] h-[450px] bg-white rounded-[2.5rem] shadow-[0_40px_100px_-15px_rgba(0,0,0,0.45)] border border-white/50 z-[10000] flex flex-col overflow-hidden animate-in slide-in-from-bottom-20 fade-in duration-1000 hover:scale-[1.02] transition-transform pointer-events-auto"
                >
            ...
            <Dialog open={isOpen} onOpenChange={onOpenChange}>
                <DialogContent className="sm:max-w-[550px] rounded-[3rem] border-none shadow-2xl p-0 overflow-hidden bg-white w-[95vw]">
                                                ...
                                                <div className="grid grid-cols-2 gap-3">
                                                    {[
                                                        ...
                                                        <button
                                                            ...
                                                            className={`relative group h-20 rounded-2xl border-2 transition-all overflow-hidden ${coverTemplate === t.id ? 'border-brand-500 ring-4 ring-brand-50' : 'border-gray-100 hover:border-brand-200'}`}
                                                        >
                                                            ...
                                                            <div className="relative h-full flex flex-col items-center justify-center gap-1">
                                                                <t.icon className={`h-5 w-5 ${coverTemplate === t.id ? 'text-brand-600' : 'text-gray-400'}`} />
                                                                <span className={`text-[9px] font-black uppercase tracking-wider ${coverTemplate === t.id ? 'text-brand-600' : 'text-gray-500'}`}>{t.label}</span>
                                                            </div>
                                                            {coverTemplate === t.id && (
                                            ...
                                            <div className="grid grid-cols-2 gap-3">
                                                {[
                                                    ...
                                                    <button
                                                        ...
                                                        className={`p-3 rounded-2xl border-2 transition-all flex flex-col items-center gap-1 ${contentTemplate === t.id ? 'border-brand-500 bg-brand-50/30 ring-4 ring-brand-50' : 'border-gray-100 hover:border-gray-200 bg-white'}`}
                                                    >
                                                        <span className={`text-[9px] font-black uppercase tracking-widest ${contentTemplate === t.id ? 'text-brand-600' : 'text-gray-900'}`}>{t.label}</span>
                                                        <span className="text-[7px] font-bold text-gray-400 uppercase leading-none">{t.desc}</span>
                                                    </button>
                                                ))}
                                            </div>
                                        </div>
